...
import { currentUser, isAdmin, deviceId } from "@/lib/auth";
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  const db = getDb();
  const c = await db
    .prepare("SELECT device_id, user_id FROM comments WHERE id = ? AND deleted_at IS NULL")
    .get<{ device_id: string; user_id: string | null }>(params.id);
  if (!c) return NextResponse.json({ error: "Nicht gefunden." }, { status: 404 });

  const user = await currentUser();
  const owner = (!!user && c.user_id === user.id) || deviceId(req) === c.device_id;
