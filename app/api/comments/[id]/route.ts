...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  const db = getDb();
  const c = db.prepare("SELECT device_id FROM comments WHERE id = ? AND deleted_at IS NULL").get(params.id) as
    | { device_id: string }
    | undefined;
  ...
  db.prepare("UPDATE comments SET deleted_at = ? WHERE id = ?").run(nowIso(), params.id);
